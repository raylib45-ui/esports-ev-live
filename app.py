...
star = st.sidebar.checkbox("Star vs Tier2? (no if tier1 vs tier1)")
...
if st.sidebar.button("Add to Pool"):
    ...
    if 0.48 <= winp <= 0.52:
        ...
        exp = cfg["base"] * (0.92 if winp >= cfg["blowout"] else 1.0)
...
if st.session_state.board:
    filtered = [x for x in st.session_state.board if x["prob"]>=0.61 and x["ev"]>=0.09]
    ...
    st.info("Add 80-120 lines from Esports -> ALL to get 10-12 edges")